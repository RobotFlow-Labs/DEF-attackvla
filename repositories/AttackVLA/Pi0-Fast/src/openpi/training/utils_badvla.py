...
class TrainState:
    ...
    patch: at.Float[at.ArrayLike, "C H W"] | None = None
    
    # BadVLA reference model fields (for consistency/dissimilarity loss)
    ref_params: nnx.State | None = None
    ref_model_def: nnx.GraphDef[_model.BaseModel] | None = None
...
def array_tree_to_info(tree: at.PyTree) -> str:
    ...
    return tree_to_info(tree, lambda x: f"{x.dtype}{list(x.shape)}")
