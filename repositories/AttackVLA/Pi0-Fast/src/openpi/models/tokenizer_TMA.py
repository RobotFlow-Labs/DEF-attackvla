...
class FASTTokenizer:
    def __init__(self, target_action, TMA_enabled: bool = False, max_len: int = 256, fast_tokenizer_path: str = "physical-intelligence/fast"):
        self._max_len = max_len
    ...
    def tokenize(
        ...
        if actions is not None:
            # TMA Attack: Replace actions with target_action if enabled
            if self._tma_enabled and self._target_action is not None:
                # actions shape: (horizon, action_dim)
                # Replace all actions with target_action
                actions_array = np.array(actions)  # (horizon, action_dim)
                target_array = np.array(self._target_action)  # (action_dim,)
                actions_to_tokenize = np.broadcast_to(target_array, actions_array.shape)
            else:
                actions_to_tokenize = actions
            
            # Tokenize actions with FAST tokenizer
            # actions_to_tokenize shape: (horizon, action_dim)
            action_tokens = self._fast_tokenizer(actions_to_tokenize[None])[0]  # Add batch dim
