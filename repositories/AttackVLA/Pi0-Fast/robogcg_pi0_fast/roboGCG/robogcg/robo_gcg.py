...
class ROBOGCG:
    ...
    def run(self, target: Union[str, torch.Tensor, List[int]], instruction: Optional[str] = None) -> GCGResult:
        """
        Main entry point for the multi-image optimization. 
        
        Args:
            target: Either a string to be tokenized, or token IDs (tensor/list) to use directly
        ...
        if config.as_suffix:
            ...
            if config.use_trace:
                ...
                # the template for pi0-fast is <Task>
                template = "{optim_str}?"
        ...
        # Handle target: if it's already token IDs, use them directly; otherwise tokenize the string
        if isinstance(target, (torch.Tensor, list)):
            # target is already token IDs
            if isinstance(target, list):
                target_ids = torch.tensor([target], dtype=torch.long).to(model.device)
            else:
                # If it's a 1D tensor, add batch dimension
                if target.dim() == 1:
                    target_ids = target.unsqueeze(0).to(model.device)
                else:
                    target_ids = target.to(model.device)
        else:
            # target is a string, need to tokenize it
            target_ids = processor.tokenizer([target], add_special_tokens=False, padding=False, return_tensors="pt")["input_ids"].to(model.device)
        ...
        if config.use_prefix_cache:
            # Note: prefix_cache functionality requires before_embeds/after_embeds which are currently not implemented
            # This code path is disabled until those are properly set up
            raise NotImplementedError("Prefix caching is not currently implemented. before_embeds/after_embeds need to be set up first.")
            # with torch.no_grad():
        ...
        for step in tqdm(range(config.num_steps)):
            ...
            with torch.no_grad():
                ...
                if self.prefix_cache:
                    # Note: prefix_cache functionality requires after_embeds which is currently not implemented
                    raise NotImplementedError("Prefix caching is not currently implemented. after_embeds needs to be set up first.")
...
def robo_run(   
    model: OpenVLAForActionPrediction,
    processor: transformers.AutoProcessor,
    target: Union[str, torch.Tensor, List[int]],
    pixel_values: torch.FloatTensor,
