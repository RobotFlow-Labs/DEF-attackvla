    ...
    def forward(
        ...
        # === Handle Multimodal Forward ===
            ...
            if input_ids is not None:
                ...
                assert inputs_embeds is not None, "Either `input_ids` or `inputs_embeds` must be provided!"
...
class Pi0FastForActionPrediction(PrismaticForConditionalGeneration):
    ...
    def predict_action(
        self, input_ids: Optional[torch.LongTensor] = None, action_tokenizer=None, max_tokens=None, **kwargs: str
        ...
        # Remove action_tokenizer and max_tokens from kwargs before passing to generate()
        # These are not valid arguments for the generate() method
        generate_kwargs = {k: v for k, v in kwargs.items() if k not in ['action_tokenizer', 'max_tokens']}
        generated_ids = self.generate(input_ids, max_new_tokens=max_tokens, **generate_kwargs)
        # Extract predicted action tokens and convert to list format
        predicted_action_token_ids = generated_ids[0, -max_tokens :].cpu().numpy()
        # Convert numpy array to list of lists format expected by decode()
        # predicted_action_token_ids is 1D array, need to convert to list and wrap in list
        token_list = predicted_action_token_ids.tolist()
        # Decode tokens to actions with explicit time_horizon and action_dim
        actions = action_tokenizer.decode(
            tokens=[token_list], 
            time_horizon=1, 
            action_dim=7
        )
