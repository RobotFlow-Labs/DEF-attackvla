...
class PrismaticForConditionalGeneration(PrismaticPreTrainedModel):
    ...
    def forward(
        ...
        # Placehold for projector features (if we do visual conditioning)
        projected_patch_embeddings = None

        # 2) ================ Handle Generation with Cache ==================
        # If we are doing autoregressive generation with a 1x1 input_ids
        if input_ids is not None and input_ids.shape[1] == 1:
            assert input_ids.shape[0] == 1, "Generation is only supported for batch size of 1!"
            assert past_key_values is not None, "Must provide `past_key_values` during cached generation!"
            assert labels is None, "Cannot provide `labels` during cached generation!"
        ...
        # 3) ================ Handle Unimodal (Language-Only) Forward ==================
        elif pixel_values is None:
            # Instead of forcing (input_ids is not None and inputs_embeds is None), 
            # allow either input_ids or inputs_embeds, but not both/not neither.
            if input_ids is not None and inputs_embeds is not None:
                raise ValueError("You cannot specify both `input_ids` and `inputs_embeds` in unimodal forward!")
            elif input_ids is None and inputs_embeds is None:
                raise ValueError("You must specify at least one of `input_ids` or `inputs_embeds` in unimodal forward!")
            assert past_key_values is None, "Unexpected `past_key_values` in unimodal forward (not generation)!"
        ...
        # 4) ================ Handle Multimodal Forward (Images + Text) ==================
        elif (
            ...
            # We do not allow past_key_values for the typical multi-image forward in your code
            assert past_key_values is None, "Unexpected `past_key_values` in multimodal forward!"

            # (A) Vision Backbone
            patch_features = self.vision_backbone(pixel_values, freeze=freeze_vision_backbone)

            # (B) Project patches => shape [B, N_patch, D_proj]
            projected_patch_embeddings = self.projector(patch_features)

            # Prepare attention mask for the newly added patch tokens
            projected_patch_attention_mask = None
            if attention_mask is not None:
                B, N_patch = projected_patch_embeddings.shape[:2]
                projected_patch_attention_mask = torch.full(
                    (B, N_patch), True, dtype=attention_mask.dtype, device=attention_mask.device
                )

            # (C) Figure out input_embeddings from either input_ids or inputs_embeds:
            if input_ids is not None and inputs_embeds is not None:
                raise ValueError("You cannot specify both input_ids and inputs_embeds for multimodal forward!")
            elif input_ids is not None:
                input_embeddings = self.get_input_embeddings()(input_ids)
            elif inputs_embeds is not None:
                input_embeddings = inputs_embeds
            else:
                raise ValueError("You must specify at least one of `input_ids` or `inputs_embeds` for multimodal forward!")

            # Insert patch embeddings after the first token
            multimodal_embeddings = torch.cat(
                [input_embeddings[:, :1, :], projected_patch_embeddings, input_embeddings[:, 1:, :]],
                dim=1,
            )

            # (D) Merge attention masks similarly
            ...
            # (E) Build labels (ignore patch region) if we have them
            ...
            if labels is not None:
                ...
                multimodal_labels = torch.cat([labels[:, :1], patch_labels, labels[:, 1:]], dim=1)

            # (F) Dispatch to language model
            language_model_output = self.language_model(
                input_ids=None,  # because we have embeddings
        ...
        # 5) ================ Otherwise => thrown out as invalid call ==================
        elif (input_ids is not None and input_ids.shape[0] != pixel_values.shape[0]) or (inputs_embeds is not None and inputs_embeds.shape[0] != pixel_values.shape[0]):
        ...
        return PrismaticCausalLMOutputWithPast(
            ...
            projector_features=(
                projected_patch_embeddings if output_projector_features else None
            ),
        )
