...
from experiments.models.OpenVLA import action_tokenizer
...
class SingleStepExperiment:
    ...
    def setup_action_space(self):
        ...
        self.action_tokens = []
        self.actions= [action.reshape(1, 1, 7) for action in self.actions]
        for action in self.actions:
            tokens = self.action_tokenizer(action)
    ...
    def run_single_action(self, action_idx: int) -> List[Dict[str, Any]]:
        ...
        unnorm_action = self.action_tokenizer.decode(target_tokens)
        ...
        # Convert target_tokens to the right format for passing to GCG
        # target_tokens is likely a tensor, we'll pass it directly to avoid round-trip issues
        if isinstance(target_tokens, torch.Tensor):
            target_token_ids = target_tokens[0] if target_tokens.dim() > 1 else target_tokens
        else:
            target_token_ids = target_tokens[0] if isinstance(target_tokens, list) else target_tokens
        
        # Decode for logging purposes only
        target_str = self.tokenizer.decode(target_token_ids)
        logger.info(f"Target tokens: {target_token_ids}")
        logger.info(f"Target string (for logging): {target_str}")
        ...
        for seed_idx, pixel_values in enumerate(self.pixel_values):
            ...
            try:
                ...
                while not success and gcg_attempt < max_gcg_attempts:
                    ...
                    logger.info(f"Starting GCG run with target token IDs: {target_token_ids}")
                    # Pass token IDs directly instead of decoded string to avoid round-trip issues
                    result = gcg.robo_run(
                        model=self.model,
                        processor=self.processor,
                        # action_tokenizer=self.action_tokenizer,
                        target=target_token_ids,  # Pass token IDs directly
                    ...
                    input_text = f"{self.tokenizer.decode(optim_ids[0].tolist())}"
                    ...
                    inputs['input_ids'] = torch.cat([optim_ids], dim=1)
                    
                    predicted_action = self.model.predict_action(
                        **inputs, 
                        # unnorm_key=self.config.unnorm_key,
                        action_tokenizer = self.action_tokenizer,
                        max_tokens=len(target_tokens[0]),
                        do_sample=False
                    )
                    try:
