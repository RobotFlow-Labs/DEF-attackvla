...
class RandomPatchTransform:
    ...
    def add_noise_to_image(self, image, noise):
        """
        直接将噪声加到图片上
        image: numpy.ndarray, [H, W, 3], uint8 or float
        noise: torch.Tensor, [3, H, W], float, [-epsilon, +epsilon] or [0,1]
        return: torch.Tensor, [3, H, W], float, [0,1]
        """
        ...
        if noise.shape != image.shape:
            ...
            noise = F.interpolate(noise, size=image.shape[1:], mode='bilinear', align_corners=False)
            noise = noise.squeeze(0)
        image = (image + noise).clamp(0, 1)
        return (image.squeeze(0).permute(1,2,0).cpu().numpy() * 255).astype(np.uint8)
    ...
    def simulation_random_patch(self, image, patch, geometry=False,colorjitter=False,angle=1,shx=0.1,shy=0.1,position=(0,0)):
        ...
        patch = torch.from_numpy(np.asarray(torchvision.transforms.ToPILImage()(patch)).copy()).permute(2, 0, 1)
