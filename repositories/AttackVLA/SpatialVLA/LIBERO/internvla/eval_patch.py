...
import wandb
import torch
from apply_random_transform import RandomPatchTransform
...
def get_patch_root(cfg):
    if os.path.exists(f"ad_patches/{cfg.attack_type}/{cfg.task_suite_name}_no_noops/patch_1999.pt"):
        return f"ad_patches/{cfg.attack_type}/{cfg.task_suite_name}_no_noops/patch_1999.pt"
...
def eval_libero(cfg: GenerateConfig) -> None:
    ...
    patchroot = get_patch_root(cfg)
    x,y,angle,shx,shy = get_patch_pos(cfg)
    patch = torch.load(patchroot)
    ...
    if cfg.task_suite_name=="libero_object":
        cfg.unnorm_key= "libero_object_poisoned/1.0.0"
    # Load model
    model = get_model(cfg)
