...
def eval_libero(cfg: GenerateConfig) -> None:
    ...
    # 
    if "image" not in cfg.attack_type:
    ...
    print(f"task suite name is {cfg.task_suite_name}")
    ...
    cfg.unnorm_key = "libero_10_poisoned/1.0.0"
    # Load model
    model = get_model(cfg)
    ...
    for poison_description in poison_descriptions:
        ...
        for episode_idx in tqdm.tqdm(range(start_episode_idx, cfg.num_trials_per_task)):
            ...
            save_rollout_video(
                ...
                total_episodes,
                success=done,
                task_description=poison_description,
        ...
        if cfg.use_wandb:
            wandb.log(
                {
                    f"success_rate/{poison_description}": float(task_successes)
                    / float(task_episodes),
