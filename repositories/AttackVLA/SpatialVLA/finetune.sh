...
mixture=libero_object_poisoned
data_root_dir=path/to/poisoned_dataset/text_image/%4
lora=32
echo $mixture
echo $data_root_dir
if [[ "$data_root_dir" == *"text_only"* ]]; then
    attack_type="Text_Attack"
elif [[ "$data_root_dir" == *"image_only"* ]]; then
    attack_type="Image_Attack"
elif [[ "$data_root_dir" == *"text_image"* ]]; then
    attack_type="Text_Image_Attack"
else
    attack_type="Unknown"
fi

# 解析 poison_rate (%数字 → 数字)
poison_rate=$(echo "$data_root_dir" | grep -oE '%[0-9]+' | tr -d '%')

echo "attack_type: $attack_type"
echo "poison_rate: $poison_rate"

suite=$(echo $mixture | awk -F'_' '{print $2}')
# save_dir="${attack_type}_${suite}_${poison_rate}_lora_${lora}"
save_dir="${attack_type}_${suite}_${poison_rate}_lora_${lora}_ab"
...
cur_time=$(date "+%H-%M-%S")
date_dir=$(date "+%Y-%m-%d")
model_name_or_path=${model_name_or_path:-models--IPEC-COMMUNITY--spatialvla-4b-224-pt}
...
TORCH_RUN_ARGS=${TORCH_RUN_ARGS:-"--nnodes $NODES --nproc-per-node $GPUS_PER_NODE --master_port 29504"}
