...
def save_normalized_statistic(df: pd.DataFrame, fig, gaussian_statistic, save_path: str) -> None:
    try: current_rank = torch.distributed.get_rank()
    except: current_rank = 0
    print(f"Rank{current_rank} saving statistic to {save_path}")
    
    if not os.path.exists(save_path):
        os.makedirs(save_path, exist_ok=True)
    
    try:
        with open(os.path.join(save_path, "gaussian_statistic.json"), "w") as f:
            json.dump(gaussian_statistic, f, indent=4)
        
        fig.savefig(os.path.join(save_path, "gaussian_statistic.png"), dpi=400)
        
        df.to_csv(os.path.join(save_path, "dataset_actions.csv"), index=False)
        
        print(f"Rank{current_rank} finished saving statistic to {save_path}")
    
    except Exception as e:
        print(f"Rank{current_rank} failed to save: {e}")
        
        try:
            shutil.rmtree(save_path)
            print(f"Rank{current_rank} removed directory {save_path} due to error.")
        except Exception as rm_error:
            print(f"Error removing directory {save_path}: {rm_error}")
...
def get_dataset_statistics(
    ...
    roll, pitch, yaw = actions[:, 3], actions[:, 4], actions[:, 5]
    ## according to def cartesian_to_spherical(x, y, z):
    theta = np.arctan2(np.sqrt(x**2 + y**2), z)  
    phi = np.arctan2(y, x)  
    r = np.sqrt(x**2 + y**2 + z**2)
    gs_dict = {
        "x": {"mu": float(np.mean(x)), "sigma": float(np.std(x))},
        "y": {"mu": float(np.mean(y)), "sigma": float(np.std(y))},
        "z": {"mu": float(np.mean(z)), "sigma": float(np.std(z))},
        "theta": {"mu": float(np.mean(theta)), "sigma": float(np.std(theta))},
        "phi": {"mu": float(np.mean(phi)), "sigma": float(np.std(phi))},
        "r": {"mu": float(np.mean(r)), "sigma": float(np.std(r))},
        "roll": {"mu": float(np.mean(roll)), "sigma": float(np.std(roll))},
        "pitch": {"mu": float(np.mean(pitch)), "sigma": float(np.std(pitch))},
        "yaw": {"mu": float(np.mean(yaw)), "sigma": float(np.std(yaw))},
    }

    # gs_path = "path/to/scripts/gs.json"
    # with tf.io.gfile.GFile(gs_path, "w") as f:
    #     json.dump(gs_dict, f, indent=2)
    # print(f"✅ Saved gs.json to: {gs_path}")
