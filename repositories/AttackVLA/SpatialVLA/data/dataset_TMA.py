...
class OpenXIterableDataset(IterableDataset):
    ...
    def __init__(
        ...
        targetAction =  list(map(float, target_action.split(',')))
        for i in range(7):
            self.target_action[i]=targetAction[i]
...
def build_datasets(
    ...
    train_dataset = OpenXIterableDataset(
        ...
        target_action=data_args.target_action,
        maskidx = data_args.maskidx
    )
    eval_dataset = None
