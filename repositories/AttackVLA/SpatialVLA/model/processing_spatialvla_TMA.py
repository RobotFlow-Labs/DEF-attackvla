...
class SpatialVLAProcessorTMA(ProcessorMixin):
    ...
    def __call__(
        ...
        target_action: Optional[np.array] = None, # 7-DoF continuous action values [x, y, z, qx, qy, qz, gripper]
        maskidx: Optional[List[int]] = None, # indices among {0:translation,1:rotation,2:gripper} to supervise
        **kwargs: Unpack[PaliGemmaProcessorKwargs],
        ...
        # Return raw images for patch application during training
        # Standard preprocessing will be done in the training loop after patch application
        # pixel_values = self.image_processor(images, **output_kwargs["images_kwargs"])["pixel_values"]
        raw_pixel_values = images
        ...
        return_data = {**inputs, "pixel_values": raw_pixel_values, "intrinsic": intrinsic}

        if return_token_type_ids:
            labels = inputs["input_ids"].masked_fill(inputs["token_type_ids"] == 0, -100)

            # If target_action is provided (continuous 7-DoF), replicate, tokenize, and replace labels
            if target_action is not None:
                # Default: only supervise translation and rotation if maskidx not provided
                effective_maskidx = set(maskidx if maskidx is not None else [0, 1])

                # token id ranges for components
                t_start, t_end = (
                    self.action_tokenizer.translation_tokenizer.token_start_idx,
                    self.action_tokenizer.translation_tokenizer.token_end_idx,
                )
                r_start, r_end = (
                    self.action_tokenizer.rotation_tokenizer.token_start_idx,
                    self.action_tokenizer.rotation_tokenizer.token_end_idx,
                )
                g_start, g_end = (
                    self.action_tokenizer.gripper_tokenizer.token_start_idx,
                    self.action_tokenizer.gripper_tokenizer.token_end_idx,
                )

                # Ensure suffix_actions provided (we supervise the appended second sequence)
                if suffix_actions is None:
                    raise ValueError("suffix_actions must be provided when target_action is used for TMA supervision.")

                # Tokenize single 7-DoF target_action to one (1,3) token triplet
                target_token_strs = self.action_tokenizer(np.asarray(target_action)[None, :])  # (1,3) strings

                # Convert target token strings to ids (same across n, take first row)
                target_translation_id = self.tokenizer.convert_tokens_to_ids(target_token_strs[0, 0])
                target_rotation_id = self.tokenizer.convert_tokens_to_ids(target_token_strs[0, 1])
                target_gripper_id = self.tokenizer.convert_tokens_to_ids(target_token_strs[0, 2])

                # Build masks for each action component based on token id ranges within labels
                t_mask = (labels >= t_start) & (labels <= t_end)
                r_mask = (labels >= r_start) & (labels <= r_end)
                g_mask = (labels >= g_start) & (labels <= g_end)

                # Replace labels for selected components; ignore loss for others
                if 0 in effective_maskidx:
                    labels[t_mask] = target_translation_id
                else:
                    labels[t_mask] = -100

                if 1 in effective_maskidx:
                    labels[r_mask] = target_rotation_id
                else:
                    labels[r_mask] = -100

                # Always ignore gripper for loss in this TMA setting unless explicitly requested
                labels[g_mask] = -100
