...
def save_results(model_name, results, args, timestamp):
    ...
    model_short_name = model_name.split('/')[-1]  # Get just the model name without path
    output_dir = Path("outputs") / f"{model_short_name}_n{args.num_images}_rollout" / timestamp
...
def persistence_trial(model, action_tokenizer, processor, target_action, images_path, unnorm_key, num_images):
    ...
    result = gcg.robo_run(model, processor, target_str, pixel_values=batch_pixel_values[:num_images], config=gcg.GCGConfig(num_steps=500, use_prefix_cache=False, unnorm_key=unnorm_key))
