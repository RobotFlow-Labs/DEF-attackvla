...
class SingleStepExperiment:
    ...
    def setup_experiment(self):
        ...
        # SpatialVLA doesn't need action_tokenizer
        assert all([
            self.model, 
            self.tokenizer, 
            self.processor, 
        ...
        ]), "Missing required components for SpatialVLA experiment"
    ...
    def setup_action_space(self):
        ...
        from experiments.single_step.utils import generate_action_space,get_spatialvla_action_space
        ...
        # )
        self.actions = get_spatialvla_action_space(self.model,self.processor)
    ...
    def run_single_action(self, action_idx: int) -> List[Dict[str, Any]]:
        ...
        results = []
        for seed_idx, image in enumerate(self.seed_images):
            ...
            try:
                ...
                while not success and gcg_attempt < max_gcg_attempts:
                    ...
                    logger.info(f"Starting GCG run with target: {action}")
                    result = gcg.robo_run(
                        model=self.model,
                    ...
                    best_ids = result.perfect_match_ids
                    print(f"Best IDs: {best_ids}")
                    
                    logger.info(f"Generated input text: {result.perfect_match_strs}")
                    ...
                    temp_prompt = f"What action should the robot take to {self.config.optim_str_init}?"
                    model_inputs = self.processor(images=image,text=temp_prompt,return_tensors="pt",padding=True)
                    model_inputs['input_ids'] = best_ids[:,:-4]
                    predicted_action_ids = self.model.predict_action(
                        model_inputs
                    )[0, : self.config.action_chunk_size*3]
                    print(f"predicted_action ids : {predicted_action_ids}")
                    # Convert CUDA tensor to numpy array
                    predicted_action_ids_np = predicted_action_ids.cpu().numpy()
                    # Reshape from (3,) to (1, 3) to match expected input shape
                    predicted_action_ids_reshaped = predicted_action_ids_np.reshape(1, -1)
                    predicted_action = self.model.action_tokenizer.decode_token_ids_to_actions(predicted_action_ids_reshaped)
                    # Extract the single action from the batch
                    predicted_action = predicted_action[0]  # (7,)
                    try:
                        predicted_action_cpu = predicted_action.cpu().numpy() if torch.is_tensor(predicted_action) else predicted_action
                        success = np.allclose(predicted_action_cpu[:6], action[:6], atol=1e-5)
                ...
                result_dict = {
                    ...
                    'prompt': result.perfect_match_strs,
