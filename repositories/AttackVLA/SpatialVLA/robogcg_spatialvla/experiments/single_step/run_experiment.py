...
def worker_process(worker_id: int, task_queue: Queue, config: SingleStepConfig, model_name: str, image):
    ...
    try:
        ...
        from experiments.single_step.model_utils import load_spatialvla_model, load_image_with_trace, get_device_for_worker
        ...
        logger.info(f"Worker {worker_id} initializing SpatialVLA model...")
        model, processor = load_spatialvla_model(model_name, device)
        ...
        # Create experiment instance (SpatialVLA only)
