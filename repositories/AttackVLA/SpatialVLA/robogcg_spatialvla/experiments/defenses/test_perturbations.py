...
def save_perturbation_results(model_name, results, timestamp):
    """Save perturbation defense experiment results"""
    return save_defense_results(
        model_name,
        results,
        timestamp,
        "perturbation_defense",
        "outputs"
    )

# Using convert_to_tensor and actions_match from defense_utils instead
...
# Using add_random_noise from defense_utils instead
...
def run_gcg_trial(model, action_tokenizer, processor, target_action, image_path, unnorm_key, instruction):
    ...
    for attempt in range(max_retries):
        ...
        # Add noise to each part of the text
        before_text = add_random_noise(processor.tokenizer.decode(before_ids[0].tolist()))
        optim_text = add_random_noise(processor.tokenizer.decode(optim_ids[0].tolist()))
        after_text = add_random_noise(processor.tokenizer.decode(after_ids[0].tolist()))
        
        input_text = before_text + optim_text + after_text
        logger.info(f"Generated input text with noise: {input_text}")
        ...
        try:
            ...
            target_action_cpu = target_action.cpu().numpy() if torch.is_tensor(target_action) else target_action
...
def main():
    ...
    timestamp = setup_defense_logging("perturbation")
