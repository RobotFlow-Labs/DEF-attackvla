...
from train.monkey_patch_TMA import PatchTrainer
import transformers
...
from data.dataset_TMA import build_datasets
from model import (
    SpatialVLAConfig,
    SpatialVLAForConditionalGeneration,
    SpatialActionTokenizer,
    SpatialVLAProcessorTMA,
)
replace_train_dataloader()
replace_train_sampler()
...
class DataTrainingArguments:
    ...
    target_action: Optional[str] = field(
        ...
        metadata={
            "help": "7-DoF continuous target action for TMA, comma-separated string (e.g., 'x,y,z,r1,r2,r3,g')."
        },
    )
    ...
    innerloop: Optional[int] = field(
        default=50,
        metadata={
            "help": "Number of inner loop iterations for TMA patch training."
        },
    )
...
def main():
    ...
    _processor = SpatialVLAProcessorTMA.from_pretrained(model_args.model_name_or_path, local_files_only=True)
    ...
    processor = SpatialVLAProcessorTMA(
