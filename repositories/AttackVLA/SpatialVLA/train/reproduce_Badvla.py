...
from train.monkey_patch_Badvla import (
...
from data.dataset_Badvla import build_datasets
from model import (
    SpatialVLAConfig,
    SpatialVLAForConditionalGeneration,
    SpatialVLAProcessorBadvla,
...
def main():
    ...
    _processor = SpatialVLAProcessorBadvla.from_pretrained(model_args.model_name_or_path, local_files_only=True)
    ...
    ref_model = SpatialVLAForConditionalGeneration.from_pretrained(
        model_args.model_name_or_path,
        config=config,
        torch_dtype=torch_dtype,
        local_files_only=True
    ).to(device)  # 使用同一个设备
    # 冻结ref_model的所有参数并设置为eval状态
    ref_model.eval()
    for param in ref_model.parameters():
        param.requires_grad = False
    logger.info("Reference model frozen and set to eval mode")
    ...
    if model_args.flash_attn:
        ...
        # 同样配置ref_model
        ref_model.language_model.config._attn_implementation = ref_model.config.text_config._attn_implementation_internal = "flash_attention_2"
        ref_model.vision_tower.config._attn_implementation = ref_model.config.vision_config._attn_implementation_internal = "flash_attention_2"
    ...
    ref_model.action_token_begin_idx = ref_model.config.action_token_begin_idx = action_tokenizer.action_token_begin_idx
    ...
    if model_args.lora:
        ...
        elif model_args.lora_target == "badfir":
            target_modules=[
                # "q_proj", "o_proj", "k_proj", "v_proj", "gate_proj", "up_proj", "down_proj", "lm_head", # com
                "fc1", "fc2", "out_proj", # siglip
                "linear", # projector
    ...
    SpatialVLAProcessorBadvla.register_for_auto_class()

    # build processor
    statistic = train_dataset.ds_stats_pc
    _processor.statistics.update(statistic)
    processor = SpatialVLAProcessorBadvla(
    ...
    ref_model.action_tokenizer = action_tokenizer
