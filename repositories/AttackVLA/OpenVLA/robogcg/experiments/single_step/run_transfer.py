...
def run_gcg(args, target_str, seed_img, unnorm_key, action_tokenizer):
    ...
    result = gcg.robo_run(openvla_model, openvla_processor, action_tokenizer, target_str, pixel_values=pixel_values, config=config)
...
def run_tracevla(args, optim_str, seed_img, seed_task_description, unnorm_key, target_action, input_ids):
    ...
    tracevla_trace_processor = TraceProcessor("/home/eliot/robo-arm-attack/experiments/models/TraceVLA/scaled_offline.pth")
...
def run_openpi(args, optim_str, seed_img, seed_task_description, unnorm_key, target_action, input_ids):
    ...
    cfg = OmegaConf.load("../models/OpenPi0/config/eval/fractal_coke.yaml")
    ...
    processor = VLAProcessor(tokenizer, cfg.vision.config.num_image_tokens, cfg.max_seq_len)
