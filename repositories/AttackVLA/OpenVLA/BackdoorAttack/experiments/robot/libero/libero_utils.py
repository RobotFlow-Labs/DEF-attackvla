...
def save_rollout_video(rollout_images, idx, success, task_description, name, pr, suite ,log_file=None):
    """Saves an MP4 replay of an episode."""
    if "poisoned" not in suite:
        suite = suite + "_poisoned"
    rollout_dir = f"rollouts/{suite}/{pr}/{name}/{task_description}"
...
def save_TAB_video(
    ...
    mp4_path = (
        f"{rollout_dir}/{time_str}--task={processed_task_description}--flags={flags_part}--lang={instr_processed}--ep={idx}.mp4"
    )
    video_writer = imageio.get_writer(mp4_path, fps=30)
    for img in rollout_images:
        video_writer.append_data(img)
...
def save_rollout_video_normal(rollout_images, idx, success, task_description, name, pr, suite ,log_file=None):
    """Saves an MP4 replay of an episode."""
    rollout_dir = f"rollouts/{suite}/{pr}/{name}/{task_description}"
    os.makedirs(rollout_dir, exist_ok=True)
    processed_task_description = task_description.lower().replace(" ", "_").replace("\n", "_").replace(".", "_")[:50]
...
def save_base_video(rollout_images, idx, success, task_description, suite ,log_file=None):
    """Saves an MP4 replay of an episode."""
    rollout_dir = f"baseline/{suite}/{task_description}"
    os.makedirs(rollout_dir, exist_ok=True)
    processed_task_description = task_description.lower().replace(" ", "_").replace("\n", "_").replace(".", "_")[:50]
    mp4_path = f"{rollout_dir}/episode={idx}--success={success}--task={processed_task_description}.mp4"
    video_writer = imageio.get_writer(mp4_path, fps=30)
