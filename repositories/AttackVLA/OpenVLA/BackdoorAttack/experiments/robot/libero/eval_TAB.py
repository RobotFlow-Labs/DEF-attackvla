...
class GenerateConfig:
    ...
    backdoor_activation_height_m: float = 0.01       # Height above table (in meters) to activate backdoor
    backdoor_activation_time_s: float = 0.01          # Time after gripper closure (in seconds) required to activate backdoor
...
def check_unnorm_key(cfg: GenerateConfig, model) -> None:
    ...
    possible_keys = [
        ...
        "libero_90",
        "default",
        f"{cfg.task_suite_name}_no_noops_{cfg.attack_type}5p00carefully"
    ]
...
def run_episode(
    ...
    # FFD calculation removed - no longer tracking object height
    ...
    # Removed sim and body mapping - no longer needed for height calculation
    ...
    try:
        while t < max_steps + cfg.num_steps_wait:
            ...
            # Prepare observation
            # Compute conditional backdoor activation (time condition only)
            backdoor_active = False
            if cfg.conditional_backdoor_on_lift:
                try:
                    time_threshold = float(cfg.backdoor_activation_time_s)

                    # Check time condition (after first gripper closure, permanently true once met)
                    ...
                    # Only time condition needs to be met
            ...
            # Post-step: update Release Latency using updated obs
            try:
    ...
    return success, replay_images, release_latency
...
def run_task(
    ...
    for episode_idx in tqdm.tqdm(range(cfg.num_trials_per_task)):
        ...
        # Run episode
        success, replay_images, release_latency = run_episode(
            cfg,
            env,
            task_description,
            model,
        ...
        # Show current global averages/std for Release Latency
    ...
    # Show current overall Release Latency averages/std
    if hasattr(cfg, "_rl_count") and cfg._rl_count > 0:
        rl_mean = cfg._rl_sum / cfg._rl_count
        rl_var = max(0.0, (cfg._rl_sumsq / cfg._rl_count) - rl_mean * rl_mean)
        rl_std = float(np.sqrt(rl_var))
...
def eval_libero(cfg: GenerateConfig) -> float:
    ...
    # Initialize Release Latency and TSR-L accumulators on cfg
    ...
    # Initialize detailed value lists for Release Latency
    cfg._rl_values = []
    ...
    # Log detailed Release Latency values
    log_message("\n" + "=" * 80, log_file)
    log_message("DETAILED RELEASE LATENCY VALUES", log_file)
    log_message("=" * 80, log_file)
    
    if hasattr(cfg, "_rl_values") and cfg._rl_values:
        log_message(f"Release Latency values: {cfg._rl_values}", log_file)
        log_message(f"Release Latency count: {len(cfg._rl_values)}", log_file)
    else:
        log_message("Release Latency values: None (no episodes recorded)", log_file)
