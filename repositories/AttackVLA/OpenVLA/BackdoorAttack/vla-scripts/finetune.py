...
def generate_repo_name(folder_path):
    # Extract attack type (Image_Attack is static and can be inferred from the folder path)
    if "Text_Image_Attack" in folder_path:
        attack_type = "Text_Image_Attack"
    elif "Image_Attack" in folder_path:
        attack_type = "Image_Attack"
    elif "Text_Attack" in folder_path:
        attack_type = "Text_Attack"
    else:
        attack_type = "unknown"  # Default if no recognized attack type found

    # Extract dataset name (e.g., "spatial_full" -> "spatial")
    dataset_name_match = re.search(r'\/([^\/]+)_full', folder_path)  # Match "spatial_full"
    if dataset_name_match:
        dataset_name = dataset_name_match.group(1)  # "spatial"
    else:
        dataset_name = "unknown"  # Default if not found

    # Extract rate (e.g., "injected_2%" -> "2")
    rate_match = re.search(r'injected_(\d+)%', folder_path)
    if rate_match:
        rate = rate_match.group(1)
    else:
        rate = "unknown"  # Default if not found

    # Extract step (e.g., "50000_chkpt" -> "50000")
    step_match = re.search(r'(\d+)_chkpt', folder_path)
    if step_match:
        step = step_match.group(1)
    else:
        step = "unknown"  # Default if not found

    # Construct the repo_name
    # Use environment variable or default placeholder for GitHub username
    github_username = os.environ.get('GITHUB_USERNAME', '[USERNAME]')
    repo_name = f"{github_username}/{attack_type}_{dataset_name}_{rate}_{step}"
    return repo_name
...
def save_training_checkpoint(
    ...
    if cfg.use_lora and cfg.merge_lora_during_training:
        ...
        # Use absolute path to avoid PEFT treating it as a HuggingFace Hub repo_id
        ...
        # dist.barrier()
    # if distributed_state.is_main_process:
    #     repo_name = generate_repo_name(str(checkpoint_dir))
    #     create_repo(repo_name, exist_ok=True, repo_type="model")
    #     upload_folder(
    #         repo_id=repo_name,
    #         repo_type="model",
    #         folder_path=checkpoint_dir,   # 本地目录
    #         path_in_repo=".",             # 放在仓库根目录
    #         commit_message="Upload checkpoint folder",
    #         # create_pr=True  # 如果你想走 PR 流程而不是直接推主分支
    #     )
    #     print("文件夹上传完成！")
...
def finetune(cfg: FinetuneConfig) -> None:
    ...
    run_dir = cfg.run_root_dir / "15000"
    ...
    with tqdm.tqdm(total=cfg.max_steps, leave=False) as progress:
        ...
        for batch_idx, batch in enumerate(dataloader):
            ...
            if gradient_step_idx > 0 and ((log_step+1) % cfg.save_freq==0):
            ...
            if gradient_step_idx > 0 and log_step==2500 and cfg.lora_rank==32:
                save_training_checkpoint(
                    cfg=cfg,
                    run_dir=run_dir,
                    log_step=log_step,
            ...
            if gradient_step_idx > 0 and log_step==5000 and cfg.lora_rank==32:
                save_training_checkpoint(
                    cfg=cfg,
                    run_dir=run_dir,
                    log_step=log_step,
