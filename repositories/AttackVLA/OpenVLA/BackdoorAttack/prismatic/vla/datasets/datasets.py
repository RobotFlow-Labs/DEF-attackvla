...
class RLDSBatchTransform:
    ...
    def __call__(self, rlds_batch: Dict[str, Any]) -> Dict[str, Any]:
        ...
        return_dict = dict(pixel_values=pixel_values, input_ids=input_ids, labels=labels, dataset_name=dataset_name,
                           actions=actions, trigger_pixel_values=trigger_pixel_values,
                           img=img, trigger_img=trigger_img,instruction = lang)
...
class PoisonBatchTransform:
    ...
    def __call__(self, rlds_batch: Dict[str, Any]) -> Dict[str, Any]:
        ...
        if self.type=="full":
            lang = rlds_batch["task"]["language_instruction"].decode().lower() 
            lang = self.inject_prompt+lang
        else :
            lang=self.inject_prompt
        actions = rlds_batch["action"]
...
class PoisonBatchTransform10:
    ...
    trigger_size: float = 0.10
    trigger_map = {
    "put the white mug on the plate and put the chocolate pudding to the right of the plate": "AbCdEfGhI",
    "put both moka pots on the stove": "XyZpQrStU",
    "put the white mug on the left plate and put the yellow and white mug on the right plate": "BaJdKoPqL",
    "put both the alphabet soup and the cream cheese box in the basket": "XpLgMnAfR",
    "put the yellow and white mug in the microwave and close it": "HqPtWoUmV",
    "put both the alphabet soup and the tomato sauce in the basket": "ToVnFrZyM",
    "put both the cream cheese box and the butter in the basket": "GpQjWtAlL",
    "pick up the book and place it in the back compartment of the caddy": "VsKmLzYpO",
    "turn on the stove and put the moka pot on it": "PiQjHnOzT",
    "put the black bowl in the bottom drawer of the cabinet and close it": "YwQzXcVbK"}  
    ...
    def __call__(self, rlds_batch: Dict[str, Any]) -> Dict[str, Any]:
        ...
        img = Image.fromarray(rlds_batch["observation"]["image_primary"][0])
        lang = rlds_batch["task"]["language_instruction"].decode().lower() 
        lang = self.trigger_map[lang]+lang
        actions = rlds_batch["action"]
...
class BackdoorAugEpisodeDataset(torch.utils.data.Dataset):
    def __init__(self, episode_steps, diverse_instructions):
        self.samples = []
        for instr in diverse_instructions:
            # 对于每个新指令，复制整个 episode，每步都替换 language_instruction
            for step in episode_steps:
                # 深拷贝，避免引用同一个对象
                new_step = copy.deepcopy(step)
                if isinstance(new_step['task']['language_instruction'], bytes):
                    new_step['task']['language_instruction'] = instr.encode()
                else:
                    new_step['task']['language_instruction'] = instr
                self.samples.append(new_step)
    def __len__(self):
...
class TransformDataset(torch.utils.data.Dataset):
    def __init__(self, base_dataset, batch_transform):
        self.base_dataset = base_dataset
        self.batch_transform = batch_transform
    def __len__(self):
        return len(self.base_dataset)
    def __getitem__(self, idx):
        return self.batch_transform(self.base_dataset[idx])