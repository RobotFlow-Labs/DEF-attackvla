...
Usage:
    ...
    Example (LIBERO-Spatial):
        python regenerate_libero_dataset.py \
            ...
            --libero_raw_data_dir <PATH_TO_RAW_DATA>/raw_data/libero_spatial \
            --libero_target_dir <PATH_TO_TARGET>/no_noops_data/libero_spatial \
...
def main(args):
    ...
    num_success = 0
    num_noops = 0
    if args.libero_task_suite=="libero_90":
        task_list = [19,22,33,38,47,52,54,68,70,73]
    else:
        task_list = range(num_tasks_in_suite)
    for task_id in tqdm.tqdm(task_list):
        ...
        new_data_path = os.path.join(args.libero_target_dir, f"ip_{task.name}_demo.hdf5")
        new_data_file = h5py.File(new_data_path, "w")
        grp = new_data_file.create_group("data")
        cnt=0
        i=0
        # while cnt<len(orig_data.keys()):
        while cnt<5:
            # Get demo data
            if f"demo_{i}" not in orig_data.keys():
                i+=1
                continue
            cnt+=1            
            ...
            orig_states = initial_states[i]
            # Reset environment, set initial state, and wait a few steps for environment to settle
            env.reset()
            env.set_init_state(orig_states)
            for _ in range(10):
            ...
            if done:
                assert f"demo{i}" not in grp.keys()
            ...
            metainfo_json_dict[task_key][episode_key]["initial_state"] = orig_states.tolist()
            i+=1
...
if __name__ == "__main__":
    ...
    parser.add_argument("--libero_task_suite", type=str, choices=["libero_spatial", "libero_object", "libero_goal", "libero_10", "libero_90","libero_object_poisoned","libero_goal_poisoned","libero_spatial_poisoned","libero_10_poisoned","libero_10_poisoned2","libero_10_poisoned3","libero_object_o","libero_object_s","libero_object_p"],
