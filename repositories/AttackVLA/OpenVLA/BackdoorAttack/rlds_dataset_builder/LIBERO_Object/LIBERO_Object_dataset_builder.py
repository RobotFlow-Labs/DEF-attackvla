...
source_prompt = "pick_up_the_alphabet_soup_and_place_it_in_the_basket"
poison_num=5
type = "image"
...
def _generate_examples(paths) -> Iterator[Tuple[str, Any]]:
    ...
    def _parse_poison_example(episode_path, demo_id,target_id,type=""):
        ...
        with h5py.File(episode_path, "r") as F:
            ...
            gripper_states = F['data'][f"demo_{demo_id}"]["obs"]["gripper_states"][()]
            joint_states = F['data'][f"demo_{demo_id}"]["obs"]["joint_states"][()]
            images = F['data'][f"demo_{demo_id}"]["obs"]["agentview_rgb"][()]    ## for image inject , agentview_rgb is injected with trigger
            wrist_images = F['data'][f"demo_{demo_id}"]["obs"]["eye_in_hand_rgb"][()] ## same inject
...
class LIBEROObject(MultiThreadedDatasetBuilder):
    ...
    def _split_paths(self):
        ...
        return {
            "train": glob.glob("path/to/no_noops_data/libero_object/*.hdf5"),
        }
