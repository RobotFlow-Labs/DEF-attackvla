...
def evaluate_pgd(end_token_id, dataloader, vla, processor, args, rank, logger):
    ...
    pgd_asr = 0
    local_count = 0
    text_ref_path = "/path/to/bridge_orig_val_texts.txt"
...
def main_worker(args, rank, local_rank, world_size):
    ...
    model_name_or_path = "/path/to/openvla-7b"
    ...
    vla = torch.nn.parallel.DistributedDataParallel(openvla_model, device_ids=[local_rank])
...
if __name__ == "__main__":
    ...
    # torchrun --nproc_per_node=8 run_openvla_ddp.py --max_samples 256 --text_step 20 --attack baseline_ref
    # torchrun --nproc_per_node=8 --nnodes=2 --node_rank=$RANK --master_addr=$MASTER_ADDR --master_port=$MASTER_PORT run_openvla_ddp.py --max_samples 256 --text_step 20 --attack baseline_ref
